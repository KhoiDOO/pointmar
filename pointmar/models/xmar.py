...
class XPointMAR(nn.Module):
    """ Point Masked Autoencoder with VisionTransformer backbone
    """
    def __init__(
        self, 
        num_points:int=1024,
        encoder_embed_dim:int=1024, encoder_depth:int=16, encoder_num_heads:int=16,
        decoder_embed_dim:int=1024, decoder_depth:int=16, decoder_num_heads:int=16,
        mlp_ratio:float=4., 
    ...
    def forward_mae_decoder(self, x: torch.Tensor, mask: torch.Tensor):
    ...
    def forward_loss(self, z: torch.Tensor, target: torch.Tensor, mask: torch.Tensor):
...
def xmar_pico_pipeline(**kwargs):
    model = XPointMARPipeline(
        encoder_embed_dim=128, encoder_depth=4, encoder_num_heads=4,
        decoder_embed_dim=128, decoder_depth=4, decoder_num_heads=4,
        mlp_ratio=4, 
        diffloss_d=3,
...
def xmar_small_pipeline(**kwargs):
    model = XPointMARPipeline(
        encoder_embed_dim=512, encoder_depth=8, encoder_num_heads=8,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=8,
        mlp_ratio=4, 
...
def xmar_large_pipeline(**kwargs):
    model = XPointMARPipeline(
        encoder_embed_dim=1024, encoder_depth=16, encoder_num_heads=16,
        decoder_embed_dim=1024, decoder_depth=16, decoder_num_heads=16,
        mlp_ratio=4, 
